...
(function() {
  ...
  // Real-time ban listener — shows ban card if email is banned while user is logged in
  (async function() {
    try {
      var firebaseConfig = await import("../firebase-config.js");
      var firestoreMod = await import("https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js");
      var db = firebaseConfig.db;
      var email = (session.email || '').trim().toLowerCase();
      if (!email) return;

      firestoreMod.onSnapshot(firestoreMod.doc(db, "bannedEmails", email), function(snap) {
        if (snap.exists()) {
          var data = snap.data();
      ...
      console.error('Ban guard listener error:', e);
    }
  })();
  ...
  function showBanCard(reason) {
    ...
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(15,23,42,0.75);backdrop-filter:blur(6px);z-index:99999;display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity 0.3s ease;';
    ...
    overlay.innerHTML =
      '<div style="background:#fff;border-radius:24px;width:90%;max-width:380px;padding:28px 24px;text-align:center;transform:scale(0.95);transition:transform 0.3s ease;box-shadow:0 25px 50px -12px rgba(0,0,0,0.35);">' +
        ...
        '<div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:14px;padding:12px 16px;margin-bottom:20px;text-align:left;">' +
          ...
          '<p style="font-size:13px;color:#DC2626;margin:0;line-height:1.5;font-weight:600;">' + reason.replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</p>' +
        '</div>' +
